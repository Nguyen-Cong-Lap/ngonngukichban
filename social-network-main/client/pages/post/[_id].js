...
function PostComments() {
  const { state: loggedUser, setState: setLoggedUser } = useContext(UserContext);
  const router = useRouter();
  const _id = router.query._id;
  ...
  useEffect(() => {
    if (_id && loggedUser?.token) {
      fetchPost();
    }
  }, [_id, loggedUser?.token]);
  ...
  const handleCommentDelete = async ({ postId, commentId }) => {
    setShowDeleteModal(false);
    try {
      const { data } = await axios.put(`/delete-comment`, { postId, commentId });
      if (data.err) {
        toast.error(data.err);
      } else {
        setPost((prev) => ({ ...prev, comments: data }));
  ...
  const handleLikeAndUnlike = async (postId, action) => {
    try {
      ...
      console.log(error);
      toast.error(error.response?.data || "Error performing action.");
    }
  };
  ...
  const sendComment = async (event) => {
    ...
    try {
      ...
      setShowCommentModal(false);
      setComment({ targetPost: "", commentContent: "" });
      setPost((prev) => ({ ...prev, comments: data.comments }));
      toast.success("Successfully added the comment");
    } catch (error) {
      console.log(error);
